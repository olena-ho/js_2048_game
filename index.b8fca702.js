...
const handleArrowPress = (e)=>{
    ...
    updateCells();
    if (game.status === Game.GAME_STATUS.WIN) winMsg.classList.remove("hidden");
    else if (game.status === Game.GAME_STATUS.LOSE) loseMsg.classList.remove("hidden");
};
const handleStart = ()=>{
    ...
    updateCells();
    if (game.status !== Game.GAME_STATUS.PLAYING) document.removeEventListener("keydown", handleArrowPress);
};
 ...
 */ class Game {
   ...
   */ restart() {
    ...
    moveUp() {
        ...
        for(let colIndex = 0; colIndex < Game.COL_LENGTH; colIndex++){
            let column = [];
            for(let rowIndex = 0; rowIndex < Game.ROW_LENGTH; rowIndex++)if (this.state[rowIndex][colIndex] !== 0) column.push(this.state[rowIndex][colIndex]);
            ...
            for(let rowIndex = 0; rowIndex < Game.ROW_LENGTH; rowIndex++)this.state[rowIndex][colIndex] = column[rowIndex];
        }
        if (!this.statesAreEqual(previousState, this.state)) this.addNewCell();
        this.checkWinLose();
    }
   ...
   */ getStatus() {
    ...
    checkMergePossible() {
        for(let rowIndex = 0; rowIndex < Game.ROW_LENGTH; rowIndex++)for(let colIndex = 0; colIndex < Game.COL_LENGTH; colIndex++){
